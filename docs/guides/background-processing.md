...
Generated TypeScript exports `JobsStatsResponse`, `JobsFailedResponse`,
`JobStatusCountResponse`, `FailedJobResponse`, and the backend-owned
`JobHistoryStatus` enum so dashboards can type these observability responses
without duplicating job status strings.
`types:export` also writes `JobManifest.ts` from the registered job runtime,
including each backend job id and resolved queue. Use `JobManifest`, `JobIds`,
`isJobName()`, `jobNameOrNull()`, `isJobQueueName()`,
`jobQueueNameOrNull()`, `isJobPayloadName()`, `jobPayloadNameOrNull()`,
`jobNames()`, `jobEntries()`, `jobQueue()`, `jobQueueNames()`, `jobNamesInQueue()`,
`jobUsesQueue()`, `jobsInQueue()`, `jobPriority()`,
`jobsWithQueuePriority()`, `jobPayloadName()`, `jobPayloadNames()`,
`jobHasPayload()`, `jobNamesWithPayload()`, `jobUsesPayload()`, and
`jobsWithPayload()` in admin tooling instead of copying job strings or joining
queue-priority/payload metadata by hand. Nullable first selectors such as
`jobFirstEntryOrNull()`, `firstJobInQueueOrNull(...)`, and
`firstJobWithPayloadOrNull(...)` keep empty dashboard states explicit without
local `... ?? null` wrappers. The same file
includes frontend-safe worker policy metadata through `JobRuntimeManifest`,
`JobDefaultQueue`, `JobMaxRetries`, `jobQueuePriority()`, and
`jobHistoryTracked()`, so dashboards can show retry, timeout, concurrency,
queue-priority, and history-retention policy without mirroring config. Worker
polling, lease, shutdown, requeue, and prune scheduling internals stay
backend-only. Runtime manifest export requires `jobs.queue` and
`jobs.queue_priorities` keys to be non-empty trimmed queue ids, with
`jobs.max_retries` and `jobs.timeout_seconds` greater than `0`.
`max_concurrent_jobs = 0` and `history_retention_days = 0` keep their documented
unlimited/keep-forever meanings.
When a job payload DTO should be visible to frontend tooling, register it with
`TsJobPayload`:

```rust
use foundry::prelude::{JobId, TsJobPayload};

foundry::inventory::submit! {
    TsJobPayload::new(
        JobId::new("send_welcome_email"),
        "SendWelcomeEmail",
    )
}
```

Jobs generated by `make:job` already derive `foundry::ts_rs::TS` and
`foundry::TS`, so the scaffolded payload type is exportable. Add the
`TsJobPayload` entry after the job is registered with the backend job registry;
the manifest exporter rejects payload metadata for unregistered jobs so
frontend tooling cannot point at stale queue contracts.

Generated `JobPayloadMap` and `JobPayload<"send_welcome_email">` then narrow
the payload shape for tooling and dashboards. Job manifest metadata is read-only
contract data; dispatching jobs still goes through backend code such as
`app.jobs()?.dispatch(...)`. Generated job manifest constants are frozen at
runtime, while job selector helpers return cloned entries so dashboards can add
local labels, sorting hints, or health annotations without mutating the
backend-owned manifest.
Generated OpenAPI documents the same `/_foundry/jobs/stats` and
`/_foundry/jobs/failed` response schemas for OpenAPI-generated clients.
...
`types:export` writes registered schedules to `ScheduleManifest.ts`, including
each schedule id, cron expression or interval milliseconds, overlap-lock flag,
environment filters, and whether lifecycle hooks are configured. Use
`ScheduleManifest`, `ScheduleIds`, `scheduleManifestEntry()`,
`scheduleNameOrNull()`, `scheduleNames()`, `scheduleEntries()`, `cronScheduleNames()`,
`cronSchedules()`, `intervalScheduleNames()`, `intervalSchedules()`,
`scheduleEnvironmentNames()`, `isScheduleEnvironmentName()`,
`scheduleEnvironmentNameOrNull()`, `scheduleNamesForEnvironment()`, `schedulesForEnvironment()`,
`scheduleNamesWithoutOverlapping()`, `schedulesWithoutOverlapping()`,
`scheduleHookNames()`, `isScheduleHookName()`, `scheduleHookNameOrNull()`, `scheduleNamesWithHook()`,
`schedulesWithHook()`, `scheduleCronExpression()`, and
`scheduleIntervalMilliseconds()` in admin tooling instead of copying scheduler
strings or hand-filtering schedule metadata. Nullable first selectors such as
`scheduleFirstEntryOrNull()`, `firstScheduleForEnvironmentOrNull(...)`, and
`firstScheduleWithHookOrNull(...)` keep empty scheduler groups explicit without
local `... ?? null` wrappers.
`SchedulerRuntimeManifest`, `SchedulerTickIntervalMs`,
`SchedulerLeaderLeaseTtlMs`, and `SchedulerShutdownTimeoutMs` are exported from
the same file so dashboards can display tick cadence, leadership lease, and
shutdown policy without mirroring scheduler config. Generated schedule
runtime export requires `scheduler.tick_interval_ms` and
`scheduler.leader_lease_ttl_ms` to be greater than `0`, while
`scheduler.shutdown_timeout_ms = 0` keeps its documented abort-immediately
meaning. Direct schedule descriptor exports must use parseable cron expressions
and positive JavaScript-safe interval milliseconds. Generated schedule constants
are frozen at runtime, while schedule selector helpers return cloned entries,
environment arrays, and hook metadata for local dashboard annotations.
...
Use `make:event --name OrderPlaced` when the payload should also be a generated
frontend contract. The scaffold derives `serde::Serialize`,
`serde::Deserialize`, `foundry::ts_rs::TS`, `foundry::TS`, and `ApiSchema`,
implements `Event`, and registers `TsEventPayload` beside the backend-owned
`EventId`. Existing hand-written events can follow the same pattern before
...
Registered listeners are also reflected in generated `EventManifest.ts` during
`types:export`, including each event id and listener count. Use `EventManifest`,
`EventIds`, `isEventName()`, `eventNameOrNull()`, `isEventPayloadName()`,
`eventPayloadNameOrNull()`, `eventNames()`,
`eventEntries()`, `eventListenerCount()`, `eventPayloadName()`,
`eventPayloadNames()`, `eventHasPayload()`, `eventNamesWithPayload()`,
`eventUsesPayload()`, `eventsWithPayload()`, `eventNamesWithListeners()`,
`eventNamesWithoutListeners()`, `eventsWithListeners()`, `eventsWithoutListeners()`,
and nullable first selectors such as `eventFirstEntryOrNull()`,
`firstEventWithPayloadOrNull()`, and `firstEventWithListenersOrNull()` in
internal tooling instead of copying event strings, listener filters, or local
`... ?? null` wrappers. Generated event manifest constants are frozen at runtime,
while event selector helpers return cloned entries for local dashboard
annotations.

Register payload DTOs with `TsEventPayload` when event payloads should be typed
for dashboards or other generated TypeScript tooling:

```rust
use foundry::prelude::{EventId, TsEventPayload};

foundry::inventory::submit! {
    TsEventPayload::new(
        EventId::new("order.placed"),
        "OrderPlaced",
    )
}
```

Generated `EventPayloadMap` and `EventPayload<"order.placed">` then narrow the
payload shape. Payload-only entries are allowed and render with
`listenerCount: 0`, which is useful for events that are emitted for audit or
tooling contracts before listeners exist.
