...
struct Pagination
  fn from_config(config: &DatabaseConfig, page: Option<u64>, per_page: Option<u64>) -> Self
  fn from_options_with_default_per_page( page: Option<u64>, per_page: Option<u64>, default_per_page: u64, ) -> Self
...
- Offset pagination links generated by `Paginated::links()`, `to_response()`,
  and `map_response()` preserve existing query parameters, replace stale
  `page` / `per_page` values, and keep URL fragments.
- The direct `Pagination` Axum extractor uses `database.default_per_page`
  when `per_page` is omitted; `Query<Pagination>` keeps the serde fallback of
  `15` because it has no app state.
